...
def init_db():
    ...
    cur.execute("""
        CREATE TABLE IF NOT EXISTS rooms (
            ...
            beds INTEGER, ac VARCHAR(10), tv VARCHAR(10),
            internet VARCHAR(10), price INTEGER, status VARCHAR(20) DEFAULT 'Available'
        )
    """)
    cur.execute("""
        CREATE TABLE IF NOT EXISTS guests (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            first_name VARCHAR(50), last_name VARCHAR(50), contact_number VARCHAR(15),
...
class LoginFrame(tk.Frame):
    def __init__(self, master):
        super().__init__(master, bg="#3c502c")
        login_container = tk.Frame(self, bg="white", padx=40, pady=40, bd=2, relief="groove")
        login_container.place(relx=0.5, rely=0.5, anchor="center")
        
        tk.Label(login_container, text="HOTEL LOGIN", font=("Arial", 24, "bold"), bg="white", fg="#7f9382").pack(pady=(0, 20))
        tk.Label(login_container, text="Username", font=("Arial", 12), bg="white").pack(anchor="w")
        self.username_entry = tk.Entry(login_container, font=("Arial", 12), width=30, bd=2)
        self.username_entry.pack(pady=(5, 15))
        tk.Label(login_container, text="Password", font=("Arial", 12), bg="white").pack(anchor="w")
        ...
        login_btn = tk.Button(login_container, text="Login", font=("Arial", 12, "bold"), 
                              bg="#349edb", fg="white", width=25, command=self.check_login)
        login_btn.pack()
...
class DashboardFrame(tk.Frame):
    def __init__(self, master):
        ...
        header = tk.Frame(self, bg="#442c50", height=100)
        header.pack(fill="x", side="top")
        tk.Label(header, text="Kings Bhavan", font=("MS Serif", 28, "bold"), 
                 bg="#482c50", fg="white").place(relx=0.5, rely=0.5, anchor="center")

        self.toolbar = tk.Frame(self, bg="white", height=140, bd=1, relief="flat")
        self.toolbar.pack(fill="x")
        self.nav_images = []
        
        buttons_info = [
            ("images\\Hotelstatus.png", "Hotel Status", self.show_status),
            ("images\\Rooms.png", "Rooms", self.show_rooms),
            ("images\\Bookroom.png", "Reserve", self.show_reserve),
            ("images\\Payments.png", "Payment & Checkout", self.show_payments),
            ("images\\PaymentHistory.png", "Payment History", self.show_payment_history),
        ...
        for i, (path, label_text, cmd) in enumerate(buttons_info):
            btn_unit = tk.Frame(self.toolbar, bg="white")
            btn_unit.pack(side="left", padx=5, pady=5)
            try:
                img = ImageTk.PhotoImage(Image.open(path).resize((50, 50), Image.Resampling.LANCZOS))
                self.nav_images.append(img)
                btn = tk.Button(btn_unit, image=img, bg="white", width=140, height=80, command=cmd, bd=0)
            except Exception:
                btn = tk.Button(btn_unit, text=label_text, bg="#ecf0f1", width=15, height=4, command=cmd)
            btn.pack()
            tk.Label(btn_unit, text=label_text, font=("msserif", 10), bg="white").pack()

        self.content_area = tk.Frame(self, bg="white", bd=2, relief="sunken")
    ...
    def show_status(self):
        ...
        tk.Label(self.content_area, text="Hotel Management Dashboard", font=("Arial", 22, "bold"), bg="white", fg="#2c3e50").pack(pady=30)
        cards_frame = tk.Frame(self.content_area, bg="white")
        cards_frame.pack(pady=10)

        def create_card(parent, title, value, color):
            card = tk.Frame(parent, bg=color, width=200, height=150, padx=20, pady=20)
            card.pack_propagate(False)
            card.pack(side="left", padx=15)
            tk.Label(card, text=title, font=("Arial", 12, "bold"), bg=color, fg="white").pack(pady=(10, 5))
            tk.Label(card, text=str(value), font=("Arial", 24, "bold"), bg=color, fg="white").pack()

        create_card(cards_frame, "Total Rooms", total_rooms, "#34495e")
        create_card(cards_frame, "Available Rooms", available_rooms, "#27ae60")
        create_card(cards_frame, "Booked Rooms", booked_rooms, "#e74c3c")
        create_card(cards_frame, "Hotel Staff", staff_count, "#2980b9")

    def show_rooms(self):
        self.clear_content()
        tk.Label(self.content_area, text="Room Availability & Details", font=("Arial", 18, "bold"), bg="white").pack(pady=10)
        filter_frame = tk.Frame(self.content_area, bg="white")
        filter_frame.pack(fill="x", padx=20, pady=5)
        tk.Label(filter_frame, text="Filter by Status:", font=("Arial", 11), bg="white").pack(side="left", padx=5)
        ...
        tk.Button(filter_frame, text="Apply Filter", command=self.refresh_room_table, bg="#3498db", fg="white", font=("Arial", 10, "bold")).pack(side="left", padx=10)
    ...
    def show_customers(self):
        self.clear_content()
        tk.Label(self.content_area, text="CUSTOMER RECORDS", font=("Arial", 20, "bold"), bg="white", fg="#2c3e50").pack(pady=20)
        search_frame = tk.Frame(self.content_area, bg="white")
        search_frame.pack(fill="x", padx=20, pady=5)
        tk.Label(search_frame, text="Search Name/Contact:", font=("Arial", 11), bg="white").pack(side="left", padx=5)
        ...
        tk.Button(search_frame, text="Search", command=self.refresh_customer_table, bg="#3498db", fg="white").pack(side="left", padx=5)
        table_frame = tk.Frame(self.content_area, bg="white")
        table_frame.pack(fill="both", expand=True, padx=20, pady=10)
    ...
    def show_contact(self):
        ...
        self.staff_photo_refs = [] # References to prevent garbage collection

        for i, staff in enumerate(staff_members):
            row, col = i // 2, i % 2
            card = tk.Frame(contact_container, bg="white", bd=1, relief="solid", padx=10, pady=10)
            card.grid(row=row, column=col, padx=20, pady=20, sticky="nsew")
            
            # Load and display staff image
            try:
                img_path = staff["img"]
                pil_img = Image.open(img_path).resize((100, 100), Image.Resampling.LANCZOS)
                tk_img = ImageTk.PhotoImage(pil_img)
                self.staff_photo_refs.append(tk_img)
                tk.Label(card, image=tk_img, bg="white").pack(side="left", padx=10)
            except Exception:
            ...
            tk.Label(info_frame, text=staff["role"], font=("Arial", 16, "bold"), bg="white", fg="black").pack(anchor="w")
            tk.Label(info_frame, text=staff["name"], font=("Arial", 11), bg="white", fg="#555").pack(anchor="w", pady=(5, 0))
            tk.Label(info_frame, text=f"Extension : {staff['ext']}", font=("Arial", 11), bg="white", fg="#555").pack(anchor="w")
            tk.Label(info_frame, text=f"Mail : {staff['mail']}", font=("Arial", 11), bg="white", fg="#3498db").pack(anchor="w")

        contact_container.grid_columnconfigure(0, weight=1)
        contact_container.grid_columnconfigure(1, weight=1)

    # --- UPDATED: Reservation Form with Adults/Children and Validation ---
    def show_reserve(self):
        self.clear_content()
        BG_COLOR, ACCENT_COLOR, BTN_COLOR = "white", "#50372c", "#27ae60"
        ...
        tk.Button(filter_frame, text="Search Rooms", bg="#3498db", fg="white", font=("Arial", 10, "bold"), command=self.search_rooms).pack(fill="x", padx=15, pady=15)
    ...
    # --- UPDATED: Processing with Validation ---
    ...
    def show_payments(self):
        self.clear_content()
        BG_COLOR, ACCENT_COLOR = "white", "#2c3e50"
        main_container = tk.Frame(self.content_area, bg=BG_COLOR)
        main_container.pack(fill="both", expand=True, padx=20, pady=20)
        ...
        tk.Button(filter_frame, text="Refresh List", bg="#3498db", fg="white", font=("Arial", 10, "bold"), command=self.refresh_occupied_rooms).pack(fill="x", padx=15, pady=5)
        ...
        tk.Button(lookup_bar, text="SEARCH GUEST", bg="#3498db", fg="white", font=("Arial", 10, "bold"), command=self.lookup_guest_for_payment).pack(side="left")
        ...
        tk.Button(actions_frame, text="COMPLETE CHECKOUT", bg="#27ae60", fg="white", font=("Arial", 12, "bold"), width=25, height=2, bd=0, command=self.finalize_payment).pack(side="left", padx=(0, 10))
    ...
    def show_payment_history(self):
        self.clear_content()
        tk.Label(self.content_area, text="PAYMENT TRANSACTION HISTORY", font=("Arial", 20, "bold"), bg="white", fg="#2c3e50").pack(pady=20)
        table_frame = tk.Frame(self.content_area, bg="white")
        table_frame.pack(fill="both", expand=True, padx=20, pady=10)
